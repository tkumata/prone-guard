...
- Wi-Fi 認証情報はビルド時設定 (`menuconfig`) で指定する。
...
1. Wi-Fi 接続
   ...
   - SSID とパスワードはビルド時に `Kconfig` から取得する。
