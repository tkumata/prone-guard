...
- [x] T-013 ランドマーク方向メトリクスを実装する。完了条件: ヨー非対称度・縦比率・短縮比が `/health` JSON に出力される。
- [x] T-014 `compute_prone_like_score` を6成分の重み付き線形結合に改良する。完了条件: ランドマーク回転傾向が `prone_like_score` に反映される。
